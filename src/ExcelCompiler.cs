...
namespace freshservice_to_servicenow_notes.src
{
  ...
  class ExcelCompiler
  {
    ...
    {
      using (var fileStream = new FileStream(this.buildDirectory + fileName + ".min.json", FileMode.Open))
      {
        using (var streamReader = new StreamReader(fileStream))
        {
          using (var jsonReader = new JsonTextReader(streamReader))
          {
            ...
            if(jsonArray == null)
            {
              Console.WriteLine("JSON NOT A LIST");
              return;
            }

            // Create a new Excel workbook as a stream
